...
export class SgeService {
  ...
  private buildPdfShell(doc: any, title: string, subtitle: string, code: string) {
    doc.fillColor('white');
    doc.rect(0, 0, doc.page.width, 72).fill(this.pdfPalette.accent);
    doc.font('Helvetica-Bold').fontSize(18).text('Sistema de Gestion de Existencias', 40, 18);
    doc.font('Helvetica').fontSize(9).text('Reporte operativo y analitico', 40, 42);
    doc.text(code, doc.page.width - 140, 18, { width: 100, align: 'right' });

    doc.roundedRect(40, 92, doc.page.width - 80, 62, 8).fillAndStroke(this.pdfPalette.accentSoft, this.pdfPalette.border);
    doc.fillColor(this.pdfPalette.ink).font('Helvetica-Bold').fontSize(16).text(title, 56, 104);
    doc.fillColor(this.pdfPalette.slate).font('Helvetica').fontSize(9).text(subtitle, 56, 126);
    doc.fillColor(this.pdfPalette.slate).fontSize(8).text(`Generado: ${this.formatDate(new Date())}`, 40, doc.page.height - 40, {
      width: doc.page.width - 80,
      align: 'right',
    });
    doc.text(`Pagina ${doc.page.number}`, 40, doc.page.height - 40, { width: 120 });
    doc.fillColor(this.pdfPalette.ink);
    // Move cursor below header/summary so subsequent content starts in the right place
    doc.y = 170;
  }

  private drawSummaryCard(doc: any, label: string, value: string, x: number, y: number, width = 160) {
    doc.roundedRect(x, y, width, 44, 6).fillAndStroke('#ffffff', this.pdfPalette.border);
    doc.fillColor(this.pdfPalette.slate).font('Helvetica').fontSize(8).text(label.toUpperCase(), x + 10, y + 8, {
      width: width - 20,
    });
    doc.fillColor(this.pdfPalette.ink).font('Helvetica-Bold').fontSize(14).text(value, x + 10, y + 22, {
      width: width - 20,
    });
    doc.fillColor(this.pdfPalette.ink);
  }
  ...
  private drawTable(
    ...
    const drawHeader = () => {
      let x = startX;
      doc.roundedRect(startX, y, tableWidth, 22, 4).fillAndStroke(this.pdfPalette.ink, this.pdfPalette.ink);
    ...
    rows.forEach((row, rowIndex) => {
      ...
      doc.roundedRect(startX, y, tableWidth, rowHeight, 3).fillAndStroke(fillColor, this.pdfPalette.border);
      let x = startX;
      doc.fillColor(this.pdfPalette.ink).font('Helvetica').fontSize(8.5);
  ...
  async getMovementsReport() {
    const [entradas, salidas] = await Promise.all([
      this.prisma.entrada.findMany({ include: { detalles: true } }),
      this.prisma.salida.findMany({ include: { detalles: true } }),
    ]);
  ...
  private async pdfFromStock(rows: any[], meta: { code: string; title: string; subtitle: string }) {
    ...
    return this.bufferFromPdf((doc) => {
      ...
      this.drawSummaryCard(doc, 'Registros', String(rows.length), 40, doc.y, 120);
      this.drawSummaryCard(doc, 'Stock total', totalStock.toFixed(2), 170, doc.y, 140);
      this.drawSummaryCard(doc, 'Criticos', String(critical), 320, doc.y, 120);
      this.drawSummaryCard(doc, 'Estado', 'Operativo', 450, doc.y, 120);
      doc.y += 58;
  ...
  async generateMovementsPdf() {
    ...
    return this.bufferFromPdf((doc) => {
      this.buildPdfShell(doc, 'Movimientos de inventario', 'Registro cronologico de entradas y salidas con detalle resumido.', 'RPT-MOV');
      doc.y = 168;

      this.drawSummaryCard(doc, 'Entradas', String(entradas.length), 40, doc.y, 120);
      this.drawSummaryCard(doc, 'Salidas', String(salidas.length), 170, doc.y, 120);
      this.drawSummaryCard(doc, 'Movimientos', String(rows.length), 300, doc.y, 120);
      this.drawSummaryCard(doc, 'Usuarios', String(new Set(rows.map((row) => row.usuario)).size), 430, doc.y, 120);
      doc.y += 58;
      ...
      this.drawTable(
        ...
        rows.map((row) => [row.type, this.formatDate(row.fecha), row.usuario || '-', row.detalle || '-']),
        {
          ...
          subtitle: 'Registro cronologico de entradas y salidas con detalle resumido.',
        },
      );
    });
  }
  ...
  async generateSuppliersPdf() {
    ...
    return this.bufferFromPdf((doc) => {
      this.buildPdfShell(doc, 'Catalogo de proveedores', 'Lista de proveedores con lead time promedio y productos asignados.', 'RPT-PROV');
      doc.y = 168;
      ...
      this.drawSummaryCard(doc, 'Proveedores', String(suppliers.length), 40, doc.y, 120);
      this.drawSummaryCard(doc, 'Productos', String(productoTotal), 170, doc.y, 120);
      this.drawSummaryCard(doc, 'Lead time prom.', `${leadPromedio} dias`, 300, doc.y, 140);
      this.drawSummaryCard(doc, 'Cobertura', 'Activa', 450, doc.y, 120);
      doc.y += 58;
      ...
      this.drawTable(
        ...
        suppliers.map((supplier) => [
          supplier.nombre,
          supplier.ruc || '-',
          `${supplier.leadTimePromedio} dias`,
  ...
  async generateAuditLogPdf() {
    ...
    return this.bufferFromPdf((doc) => {
      this.buildPdfShell(doc, 'Auditoria del sistema', 'Eventos recientes registrados por el sistema y usuarios autenticados.', 'RPT-AUD');
      doc.y = 168;

      this.drawSummaryCard(doc, 'Eventos', String(audit.length), 40, doc.y, 120);
      this.drawSummaryCard(doc, 'Usuarios', String(new Set(audit.map((row: any) => row.usuario?.usuario)).size), 170, doc.y, 120);
      this.drawSummaryCard(doc, 'Ultimo evento', audit.length ? this.formatDate(audit[0].fechaHora) : 'N/A', 300, doc.y, 160);
      this.drawSummaryCard(doc, 'Trazabilidad', 'Completa', 470, doc.y, 100);
      doc.y += 58;
